...
class JikanAPI:
    ...
    def get_top_anime(
        self, page: int = 1, filter_type: str = "bypopularity"
    ) -> Dict[str, Any]:
        """
        ...
        params = {"page": page, "filter": filter_type, "limit": config.api.batch_size}
        ...
        while retries <= config.api.max_retries:
            try:
                ...
                )  # Timeout hardcoded or add to config
                ...
                if response.status_code == 429:
                    ...
                    if retries > config.api.max_retries:
                        logger.error(
                            f"Max retries ({config.api.max_retries}) exceeded for page {page}."
                        )
                        return {}

                    logger.warning(
                        f"Rate limited (429). Retrying {retries}/{config.api.max_retries} in {self.delay}s..."
                    )
...
def parse_anime_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "Image URL": item.get("images", {})
        .get("jpg", {})
        .get("large_image_url", "N/A"),
    }
...
def build_anime_dataset(start_page: int = 1, limit_pages: int = 5) -> pd.DataFrame:
    ...
    for page in tqdm(
        range(start_page, start_page + limit_pages), desc="Fetching Pages"
    ):
...
def merge_datasets(master_df: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    ...
    master_df["mal_id"] = pd.to_numeric(master_df["mal_id"], errors="coerce").astype(
        "Int64"
    )
    ...
    logger.info(
        f"Merged: {len(master_preserved)} retained + {len(new_df)} updated/new = {len(merged_df)} total records."
    )
    return merged_df
...
def main():
    ...
    logger.info(
        f"Pipeline complete. Master database updated with {len(new_data_df)} fetched records."
    )
